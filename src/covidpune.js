...
fetch("https://covidpune.com/data/covidpune.com/bed_data.json")
  ...
  .then(function (bedJson) {
    ...
    for (var key in bedJson) {
      ...
      rowJson["COVID_beds"] = rowdata["available_beds_without_oxygen"] + "/" + rowdata["total_beds_without_oxygen"];
      rowJson["Oxygen_beds"] = rowdata["available_beds_with_oxygen"] + "/" + rowdata["total_beds_with_oxygen"];
      rowJson["ICU"] = rowdata["available_icu_beds_without_ventilator"] + "/" + rowdata["total_icu_beds_without_ventilator"];
      rowJson["Ventilator_Beds"] = rowdata["available_icu_beds_with_ventilator"] + "/" + rowdata["total_icu_beds_with_ventilator"];
