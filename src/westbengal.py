...
def getPaginatedPages(web_driver, html, html_text, district):
    soups = BeautifulSoup(html, 'html.parser')
    tb = soups.find('tbody')
    pagination = tb.find('tr', {'class': 'pagination-ys'}) if tb is not None else None
    if pagination:
        ...
        for i in range(2, len(tds)):
            if tds[i].find('a'):
                ...
                html_text[district].append(html_p)
...
if __name__ == '__main__':
    ...
    for district in districts:
        ...
        html_text[district].append(html_govt_pvt)
        # check pagination
        getPaginatedPages(driver, html_govt_pvt, html_text, district)
        ...
        html_text[district].append(html_pvt)
        getPaginatedPages(driver, html_pvt, html_text, district)
        ...
        html_text[district].append(html_govt)
        getPaginatedPages(driver, html_govt, html_text, district)
